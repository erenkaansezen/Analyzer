...
# Bütün kullanılabilir locale'leri oluştur (örneğin: 'en'-'en', 'tr'-'tr')
LOCALES = [
...
def scrape_app_reviews(app_id: str, max_reviews: int = None) -> pd.DataFrame:
    ...
    for lang, country in LOCALES:
        # reviews_all count=None ise tümünü alır, int ise o kadarını
        batch = reviews_all(
            app_id,
            lang=lang,
            country=country,
        ...
        if batch:
            # eğer bu locale'de yorum geldiyse ekle
            for r in batch:
                r["_lang"] = lang
                r["_country"] = country
            all_reviews.extend(batch)
        # eğer hiç gelmediyse sonraki locale'a geç
    if not all_reviews:
        raise RuntimeError(f"⚠️ {app_id} için hiç yorum bulunamadı.")
    df = pd.DataFrame(all_reviews)
    df["scrape_time"] = datetime.utcnow()
    # reviewText → content normalize
